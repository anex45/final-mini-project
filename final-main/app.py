...
"""
Flask REST API for Python Interview System
...
if auth_available:
    ...
    print("Authentication module not available, running in open API mode")
...
def get_question():
    ...
    Returns:
        JSON object with question text and category
    """
...
def submit_answer():
    ...
    Returns:
        JSON object with score and feedback
    """
    data = request.json
...
def end_interview():
    ...
    Request Body:
        candidateName (str): Name of the interview candidate
        results (array): Array of question/answer results
        
    Returns:
        JSON object with success status and saved filename
    """
    data = request.json
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': f'Failed to save interview results: {str(e)}'
        }), 500
...
def auth_status():
    ...
    Returns:
        JSON object with authentication status and user info if authenticated
    """
    if not auth_available:
        return jsonify({
            ...
            'data': {
                ...
                'authMode': 'open',
                'message': 'Authentication is disabled, all requests are allowed'
            }
        })
...
# No static files routes needed for API-only backend

# Add API endpoint for health check
...
def main():
    ...
    print("Starting Python Interview System REST API")
    print(f"API is running on http://0.0.0.0:{port}")
    print("Ready to be integrated with any frontend client")
